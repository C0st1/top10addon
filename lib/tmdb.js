...
// Fixes SEC-05: API key NEVER appended to URL query string (actually enforced)
 ...
 * This is the ONLY method used — API keys are NEVER in URLs.
...
// SEC-05 REMOVED: appendApiKey() deleted.
// API keys MUST go via Authorization header to prevent leakage in logs,
// browser history, referrer headers, and URL scraping.
...
async function _matchTMDBInternal({ item, type, apiKey, overrides, title, year, cacheKey }) {
    try {
        ...
        if (overrides[cleanTitleLower]) {
            ...
            if (/^tt\d+$/.test(imdbId)) {
                // SEC-05 FIX: API key in Authorization header, NOT in URL
                const overrideUrl = `https://api.themoviedb.org/3/find/${imdbId}?external_source=imdb_id`;
                const res = await fetchWithTimeout(overrideUrl, { headers: getTmdbHeaders(apiKey) }, DEFAULTS.TMDB_TIMEOUT);
                if (res.ok) {
        ...
        // SEC-05 FIX: API key in Authorization header, NOT in URL
        const sRes = await fetchWithTimeout(searchUrl, { headers: getTmdbHeaders(apiKey) }, DEFAULTS.TMDB_TIMEOUT);
        if (!sRes.ok) {
        ...
        if (sData.results?.length > 0) {
            ...
            if (cachedImdb) {
                ...
                try {
                    ...
                    // SEC-05 FIX: API key in Authorization header, NOT in URL
                    const extRes = await fetchWithTimeout(detailUrl, { headers: getTmdbHeaders(apiKey) }, DEFAULTS.TMDB_TIMEOUT);
                    if (extRes.ok) {
...
async function validateTmdbKey(apiKey) {
    ...
    try {
        ...
        // SEC-05 FIX: API key in Authorization header, NOT in URL
        const r = await fetchWithTimeout(url, { headers: getTmdbHeaders(apiKey.trim()) }, DEFAULTS.TMDB_TIMEOUT);
