...
async function fetchFlixPatrolTitles(categoryType, country = "Global") {
    ...
    try {
        ...
        // ============================================================
        // FlixPatrol HTML structure differs between global and country pages
        // 
        // GLOBAL PAGE (/netflix/):
        // - Uses section IDs: #toc-netflix-tv-shows, #toc-netflix-movies
        // - These sections contain all titles for that category
        //
        // COUNTRY PAGES (/netflix/united-states/, etc.):
        // - NO section IDs
        // - Uses <h3> headers: "TOP 10 Movies", "TOP 10 TV Shows"
        // - Each h3 is inside a .card div that contains a table with titles
        // ============================================================
        ...
        if (titles.length === 0) {
            // Look for h3 containing the header text
            $('h3').each((i, el) => {
                if (titles.length >= 10) return false; // Stop if we have enough
