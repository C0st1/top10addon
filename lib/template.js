...
// Fixes PERF-03: HTML cached at module level (countries list is static)
// ============================================================

const { escapeHtml, escapeJs } = require('./utils');
const { FLIXPATROL_COUNTRIES } = require('./constants');

// PERF-03 FIX: Pre-build the HTML template at module load time.
// The countries list is static, so the entire HTML is invariant.
// This avoids rebuilding a ~570-line template string on every request.
let _cachedConfigHTML = null;

function _buildConfigHTMLString(countries, nonce) {
    ...
    <div class="card-body">
        ...
        <div class="field">
            <label><span class="required-dot"></span> TMDB API Key
                <div class="tooltip-container">
                    <a href="https://www.themoviedb.org/settings/api" target="_blank" rel="noopener noreferrer" class="tooltip-icon">?</a>
                    <div class="tooltip-content">Required for metadata.<br>Get yours free at <b style="color:#5c9eff;">themoviedb.org</b></div>
                </div>
            </label>
            <div class="input-with-icon">
                <input type="password" id="tmdbKey" placeholder="e.g. 8a7f3bc2d1..." autocomplete="off">
                <button class="toggle-pwd" id="toggleTmdbPwd" title="Show/Hide" type="button">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                </button>
            </div>
            <div style="margin-top:6px;"><button class="btn btn-sm" id="testKeyBtn" type="button">Test Key</button> <span id="keyStatus" class="key-status"></span></div>
        </div>
            ...
            <div class="advanced-settings-content">
                <div class="field">
                    <label>RPDB API Key <span class="optional-tag">optional</span>
                        <div class="tooltip-container">
                            <a href="https://ratingposterdb.com/" target="_blank" rel="noopener noreferrer" class="tooltip-icon">?</a>
                    ...
                    <div class="input-with-icon">
                        <input type="password" id="rpdbKey" placeholder="e.g. t1-xxxxxx..." autocomplete="off">
                        <button class="toggle-pwd" id="toggleRpdbPwd" title="Show/Hide" type="button">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                        </button>
                    </div>
                    <div style="margin-top:6px;"><button class="btn btn-sm" id="testRpdbBtn" type="button">Test Format</button> <span id="rpdbStatus" class="key-status"></span></div>
                </div>
                <div class="field" style="margin-bottom: 0;">
                    ...
                    <div class="field-row" style="margin-bottom:0; gap:10px;">
                        <input type="text" id="movieOverride" placeholder="Movie Tab Name (e.g. Films)" maxlength="50">
                        <input type="text" id="seriesOverride" placeholder="Series Tab Name (e.g. TV Shows)" maxlength="50">
                    </div>
                </div>
            </div>
        </details>
        <div class="btn-group">
            <button class="btn btn-primary" id="generateBtn" type="button" style="flex: 2;">Generate Install Link</button>
            <button class="btn btn-danger" id="resetBtn" type="button" style="flex: 1;" title="Clear Configuration">Clear</button>
        </div>
        <div id="resultArea">
            <p style="font-size:12px;color:#999;">Manifest URL (credentials secured server-side):</p>
            <div class="url-box"><span class="url-box-text" id="manifestDisplayUrl"></span><button class="copy-btn" id="copyBtn" type="button">Copy</button></div>
            <div class="toast" id="copyToast">Copied to clipboard!</div>
            <button class="btn btn-secondary" id="installBtn" type="button">Install to Stremio</button>
        </div>
    </div>
</div>
<div class="footer">
    Netflix Top 10 Stremio Addon &nbsp;&bull;&nbsp; <a href="https://github.com/C0st1/top10addon" target="_blank" rel="noopener noreferrer">View on GitHub</a>
</div>
<script nonce="${nonce}">
(function() {
    'use strict';
...
/**
 * Build (or return cached) configuration page HTML.
 * PERF-03 FIX: Countries list is static, so HTML is built once and cached.
 * SEC-08 FIX: CSP nonce is injected into the script tag.
 *
 * @param {string[]} countries
 * @param {string} nonce - CSP nonce for inline script
 * @returns {string}
 */
function buildConfigHTML(countries, nonce = '') {
    // SEC-08 FIX: Cannot cache when nonce is dynamic — rebuild per request
    // The overhead is minimal (~570 lines of string concat)
    return _buildConfigHTMLString(countries, nonce);
}
